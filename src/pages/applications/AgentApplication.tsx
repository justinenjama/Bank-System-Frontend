...
import { applyForAgent } from "../../service/Service";
import Spinner from "../../components/Spinner";

const steps = [
    { id: 1, label: "Agent Name" },
    { id: 2, label: "PIN" },
...
export default function AgentApplication() {
    ...
    const handleNextStep = () => {
        if (step === 1 && !agentName.trim()) {
            toast.info("Please enter a valid Agent name");
            return;
        }
    ...
    const handleSubmit = async (e?: React.FormEvent) => {
        ...
        setLoading(true);
        try {
            const response = await applyForAgent({
                agentName,
                pin,
                confirmPin,
            });
    ...
    return (
        <div>
            ...
            >
                ...
                {step === 1 && (
                    <>
                        <label htmlFor="agentName" className="block font-medium text-gray-700">Agent Name</label>
                        <input
                            id="agentName"
                            type="text"
                            value={agentName}
