...
import { applyForPayBill } from "../../service/Service";
import Spinner from "../../components/Spinner";

const steps = [
    { id: 1, label: "Paybill Name" },
    { id: 2, label: "PIN" },
    { id: 3, label: "Confirm PIN" },
];

export default function PayBillApplication() {
    ...
    const handleNextStep = () => {
        if (step === 1 && !payBillName.trim()) {
            toast.info("Please enter a valid Paybill name");
            return;
        }
    ...
    const handleSubmit = async (e?: React.FormEvent) => {
        ...
        setLoading(true);
        try {
            const response = await applyForPayBill({
                payBillName,
                pin,
                confirmPin,
            });
    ...
    return (
        <div>
            ...
            >
                ...
                {step === 1 && (
                    <>
                        <label htmlFor="paybillName" className="block font-medium text-gray-700">Paybill Name</label>
                        <input
                            id="paybillName"
                            type="text"
                            value={payBillName}
