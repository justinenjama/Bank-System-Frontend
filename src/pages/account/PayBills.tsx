...
import { payBills, validatePayBillNumber } from "../../service/Service";
...
export default function PayBillsComponent() {
    ...
    const handleNextStep = async () => {
        if (step === 1) {
            if (payBillNumber.trim() === "") {
                toast.info("Please enter paybill number");
                return;
            }
            setLoading(true);
            try {
                const result = await validatePayBillNumber(payBillNumber);
                if (!result.exists) {
                    toast.error("Pay bill number does not exist");
                    return;
                }
                setStep(2);
            } catch (err) {
                toast.error("Failed to validate pay bill. Try again.");
            } finally {
                setLoading(false);
            }
        } else if (step === 2) {
    ...
    const handleSubmit = async (e?: React.FormEvent) => {
        ...
        setLoading(true);
        try {
            const response: PayBillResponse = await payBills({
                payBillNumber,
                amount,
                pin: pinNumber,
            });
    ...
    return (
        <div>
            ...
            >
                ...
                {step === 1 && (
                    <>
                        <label className="block font-medium text-gray-700">PayBill Number</label>
                        <input
                            type="text"
                            value={payBillNumber}
